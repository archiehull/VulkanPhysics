...
Cylinder::Cylinder(const glm::vec3& p1, const glm::vec3& p2, float radius)
	: Collider(p1), m_p2(p2), m_radius(radius)
{
}

bool Cylinder::IsInside(const glm::vec3& point) const
{
...
bool Cylinder::Intersects(const Line& line) const 
{ 
	return false; 
}

glm::vec3 Cylinder::ClosestPointOnSegment(const glm::vec3& a, const glm::vec3& b, const glm::vec3& p)
{
	glm::vec3 ab = b - a;
